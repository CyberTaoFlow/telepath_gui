...
var navMargin=(width-$('.tele-header-right').width()-$('.tele-logo').width()-$('.tele-nav').width())/2;$('.tele-nav').css('marginLeft',navMargin);if(width<801){$('.tele-nav').css('marginLeft',20);$('.tele-panel-subtitle').css('height','100px !important');$('.tele-panel-title').css('position','absolute');}}}
function RangeRemove(that){numRanges=$(that).parent().parent().children('.tele-ip-wrap').length;if(numRanges>1)
{$(that).parent().remove();}}
function RangeRemoveLB(that){numRanges=$(that).parent().parent().children('.tele-ip-wrap-lb').length;if(numRanges>1)
{$(that).parent().remove();}}
...
var ipAdd=$('<div>').addClass('tele-ip-add').addClass('tele-icon').addClass('tele-icon-plus').hover(function(){$(this).addClass('hover');},function(){$(this).removeClass('hover');}).click(function(){that.limitRanges.append(getRangeUI());});var ipRemove=$('<div>').addClass('tele-ip-remove').addClass('tele-icon').addClass('tele-icon-minus').hover(function(){$(this).addClass('hover');},function(){$(this).removeClass('hover');}).click(function(){$(this).parent().remove();});var ipToggle=$('<div>').toggleFlip({left_value:'Single',right_value:'Range',flip:function(){ipEnd.toggle();ipDash.toggle();},flipped:is_range,});ipWrap.append(ipAdd).append(ipRemove).append(ipToggle).append(ipStart).append(ipDash).append(ipEnd);return ipWrap;}
function getRangeUI(data,container){if(!data){data='';}
var is_range=data.from!=data.to;var ipWrap=$('<div>').addClass('tele-ip-wrap');var ipStart=$('<div>').addClass('tele-ip').ip({data:data.from});var ipDash=$('<div>').addClass('tele-ip-dash').html('_');var ipEnd=$('<div>').addClass('tele-ip').ip({data:!is_range?'':data.to});if(!is_range){ipDash.hide();ipEnd.hide();}
var ipAdd=$('<div>').addClass('tele-ip-add').addClass('tele-icon').addClass('tele-icon-plus').hover(function(){$(this).addClass('hover');},function(){$(this).removeClass('hover');}).click(function(){container.append(getRangeUI());});var ipRemove=$('<div>').addClass('tele-ip-remove').addClass('tele-icon').addClass('tele-icon-minus').hover(function(){$(this).addClass('hover');},function(){$(this).removeClass('hover');}).click(function(){RangeRemove(this);});var ipToggle=$('<div>').toggleFlip({left_value:'Single',right_value:'Range',flip:function(){ipEnd.toggle();ipDash.toggle();},flipped:is_range});ipWrap.append(ipAdd).append(ipRemove).append(ipToggle).append(ipStart).append(ipDash).append(ipEnd);return ipWrap;}
...
function getRangeLB(data,container){if(!data){data='';}
var is_range=data.from!=data.to;var ipWrap=$('<div>').addClass('tele-ip-wrap-lb');var ipStart=$('<div>').addClass('tele-ip').ip({data:data.from});var ipDash=$('<div>').addClass('tele-ip-dash').html('_');var ipEnd=$('<div>').addClass('tele-ip').ip({data:!is_range?'':data.to});if(!is_range){ipDash.hide();ipEnd.hide();}
var ipAdd=$('<div>').addClass('tele-ip-add').addClass('tele-icon').addClass('tele-icon-plus').hover(function(){$(this).addClass('hover');},function(){$(this).removeClass('hover');}).click(function(){container.append(getRangeLB());});var ipRemove=$('<div>').addClass('tele-ip-remove').addClass('tele-icon').addClass('tele-icon-minus').hover(function(){$(this).addClass('hover');},function(){$(this).removeClass('hover');}).click(function(){RangeRemoveLB(this);});var ipToggle=$('<div>').toggleFlip({left_value:'Single',right_value:'Range',flip:function(){ipEnd.toggle();ipDash.toggle();},flipped:is_range});ipWrap.append(ipAdd).append(ipRemove).append(ipToggle).append(ipStart).append(ipDash).append(ipEnd);return ipWrap;}
...
function escapeHtml(str){var div=document.createElement('div');div.appendChild(document.createTextNode(str));return div.innerHTML;};function unescapeHtml(escapedStr){var div=document.createElement('div');div.innerHTML=escapedStr;var child=div.childNodes[0];return child?child.nodeValue:'';};telepath.ds={get:function(resource,params,success,error,flag){var dataType='json';var method='POST';$.ajax({type:method,url:telepath.controllerPath+resource,data:params,flag:flag,success:function(data){if(data.logout){window.location.reload(true);return;}
...
var searchSettingsObj={search:this.searchStr,options:this.options,range:telepath.range,apps:telepath.appFilter,is_country:this.countryFlag,sort:this.sort,dir:this.dir};that.count=0;$.each(['alerts','cases','suspects','requests'],function(i,type){if(that.options[type]){telepath.ds.get('/search/'+type,searchSettingsObj,function(data){that.count++;that.container=$('#tele-search-'+type);that.container.empty();if(!data.items||data.items.length==0){var p=$('<p>').text("No results");that.container.append(p);if(that.count==4)
...
that.selectTab();that.container=$('#tele-search-'+type);that.container.empty();var p=$('<p>').text("No select option "+type);that.container.append(p);}});},selectTab:function(){var that=this;that.tabsEl.children('.tele-loader').remove();var found=false;$.each(['alerts','cases','suspects','requests'],function(i,type){if(!found){if(that.results[type]&&that.results[type].length>0){var select;switch(type){case'alerts':select=0;that.showAlertsTab();break;case'cases':select=1;that.showCasesTab();break;case'suspects':select=2;that.showSuspectsTab();break;case'requests':select=3;that.showRequestsTab();break;}
...
return;this.list=$('<div>').addClass('tele-requests-block');$('#tele-search-requests').append(this.list);this.list.teleList({data:this.results.requests,searchkey:this.searchStr,formatter:function(item){return telepath.suspects.rowFormatter(item);}});}}
...
var innerWidth=$(this.element).width()-offset;$('.tele-listitem-inner',this.element).width(innerWidth-15);var offset=15;if(this.options.progbar){if(this.options.progbarBig){offset=offset+$('.tele-listitem-bigprogbar',this.element).outerWidth()+55;}else{offset=offset+$('.tele-listitem-progbar',this.element).outerWidth()+25;}}
...
break;case'user':if(detail.key&&detail.key=='user'&&detail.value&&detail.value!=''){var iconEl=$('<div>').addClass('tele-listitem-user-name');li.append(iconEl).append('&nbsp&nbsp;').append(escapeHtml(detail.value));}else{li=false;}
break;case'Email':var mailTo=$('<a>').attr('href','mailto:'+escapeHtml(detail.value)).text(escapeHtml(detail.value));li.append(mailTo);break;case'country':if(detail.value.length==2){li.append('<span class="flag flag-'+escapeHtml(detail.value)+'"></span>');li.append('<span class="tele-country">'+telepath.countries.a2n(detail.value)+'</span>');}else{li.append(detail.key);if(detail.value){li.append(':&nbsp;');}
li.append('<b>'+escapeHtml(detail.value)+'</b>');}
break;case'alerts':if(parseInt(detail.value)>0){var iconEl=$('<div>').addClass('tele-listitem-icon').addClass('tele-icon-alert');var countEl=$('<div>').addClass('tele-listitem-count').html(escapeHtml(detail.value));li.addClass('tele-listitem-alerts-count');li.append(iconEl).append(countEl);}else{li=false;}
break;case'actions':if(parseInt(detail.value)>0){var iconEl=$('<div>').addClass('tele-listitem-icon').addClass('tele-icon-actions');var countEl=$('<div>').addClass('tele-listitem-count').html(escapeHtml(detail.value));li.addClass('tele-listitem-actions-count');li.append(iconEl).append(countEl);}else{li=false;}
break;case'cases':if(parseInt(detail.value)>0){var iconEl=$('<div>').addClass('tele-listitem-icon tele-icon-case');var countEl=$('<div>').addClass('tele-listitem-count').html(escapeHtml(detail.value));li.addClass('tele-listitem-cases-count');li.append(iconEl).append(countEl);}else{li=false;}
...
return result;},callbacks:{click:function(widget){$('.popover').remove();setTimeout(function(){$('.popover').remove();},1000);var parent_parent=widget.element.parent().parent(),names=(widget.options.raw.alerts_names)?widget.options.raw.alerts_names:[];telepath.sessionflow.init(widget.options.itemID,widget.options.raw.ip_orig,parent_parent,widget.options.icon,names,widget.options.raw.searchkey);},hover_in:function(el,item){if(!item){return;}
...
if(telepath.cases.pop){telepath.cases.pop.remove();}},favorite:function(widget){telepath.ds.get('/cases/set_case_fav',{cid:widget.options.title,favorite:widget.options.favorite},function(data){});}}}
...
telepath.range.end=parseInt(new Date().getTime()/1000);that.options.start=telepath.range.start;that.options.end=telepath.range.end;that.options.change(telepath.range.start,telepath.range.end);that._update();});}}});$(".tele-daterange-buttons .tele-button-cancel").click(function(){$(".tele-daterange-popup").fadeOut();});this.updateUI();},_update:function(){this.updateUI();},setPeriod:function(){var that=this;var from_date=new Date();$('.tele-darerange-container').addClass('disabled');if(that.options.state=='range'){$('.tele-darerange-container').removeClass('disabled');telepath.ds.get('/telepath/set_full_time_range',{},function(data){telepath.range.start=data.items.start;telepath.range.end=data.items.end;that.options.start=telepath.range.start;that.options.end=telepath.range.end;});}
$(".tele-daterange-to").val(date_format('d/m/Y',telepath.range.end));$(".tele-daterange-to-hour").val(date_format('H:i',telepath.range.end));$(".tele-daterange-from").val(date_format('d/m/Y',telepath.range.start));$(".tele-daterange-from-hour").val(date_format('H:i',telepath.range.start));},updateUI:function(){var that=this;if(this.button){this.button.remove();}
...
$(this).addClass("active");});$('.tele-darerange-container').addClass('disabled');if(telepath.range.state=='range'){var text=date_format('d/m/Y',this.options.start)+' - '+date_format('d/m/Y',this.options.end);$('.tele-darerange-container').removeClass('disabled');}
else if(telepath.range.state=='data')
var text='All Data';else
var text='Last '+telepath.range.state[0].toUpperCase()+telepath.range.state.slice(1);this.button=$('<a>').attr('href','#').btn({icon:'daterange',text:text,callback:function(){that._displayPopup(this);}});this.element.append(this.button);$(".tele-daterange-from").val(date_format('d/m/Y',this.options.start));$(".tele-daterange-from-hour").val(date_format('H:i',this.options.start));$(".tele-daterange-to").val(date_format('d/m/Y',this.options.end));$(".tele-daterange-to-hour").val(date_format('H:i',this.options.end));}});$.widget("tele.flotGraph",{options:{data:[],options:[],dashboard:false,title:false,},_create:function(){this.element.addClass("tele-graph");this._update();},_setOption:function(key,value){this.options[key]=value;this._update();},_update:function(){var that=this;this.canvasOuter=$('<div>').addClass('tele-graph-canvas-outer');this.canvasInner=$('<div>').addClass('tele-graph-canvas');this.element.empty();if(this.options.title){var graphTitle=$('<div>').addClass('tele-graph-title').html(this.options.title);this.element.append(graphTitle);}
...
$('.tele-overlay').resize(function(){telepath.overlay.resize();});telepath.overlay.resize();$(this.overlayEl).draggable({handle:this.headerEl});}}
...
for(x in this.tabs){var tab=this.tabs[x];tab.callback=function(widget){$('.tele-config-tab .active').removeClass('active');$('.tele-icon',widget.element).addClass('active');$('.tele-button-text',widget.element).addClass('active');var id=widget.options.icon;that.currentPanel=id;that.container.empty();$("#file-upload").hide();telepath.config[id].container=that.container;telepath.config[id].initLayout=that.initLayout;telepath.config[id].resizeLayout=that.resizeLayout;telepath.config[id].initLayout();telepath.config[id].resizeLayout();telepath.config[id].init();$(window).resize(function(){telepath.config[id].resizeLayout();});}
...
$('.tele-panel-subtitle').outerHeight();$('.tele-panel-case .tele-block').height(offset-40);$('.tele-panel-case .tele-block .tele-list').height(offset-70);$('.tele-panel-case .tele-case-alerts-block .tele-list').mCustomScrollbar("update");$('.tele-panel-case .tele-case-similar-block .tele-list').mCustomScrollbar("update");}}
telepath.cases={sort:'date',dir:false,data:[],searchString:'',init:function(){this.drawUI();this.refresh();},refresh:function(callback){var that=this;telepath.ds.get('/cases/get_cases',{search:this.searchString,sort:this.sort,dir:this.dir},function(data){var cases=[]
var index=0;for(index=0;index<data.items.length;++index){if(data.items[index].name)
{cases.push(data.items[index]);}}
that.setData(cases);if(typeof(callback)=='function'){callback();}});},setData:function(data){this.container=$('.tele-panel-cases .tele-panel-cases-inner');var that=this;this.data=data;$('.tele-block',this.container).remove();this.list=$('<div>');this.container.append(this.list);this.list.teleList({data:this.data,formatter:function(item){item.checkable=true;item.favorites=true;item.favorite=item.case_data.favorite=='1';return telepath['case'].rowFormatter(item);},callbacks:telepath.listitem.generic.callbacks_case});this.panelTitle.html(this.data.length+' Cases');setTimeout(function(){that._resize();},0);$(window).resize(function(){that._resize()});},_resize:function(){if($('.tele-panel-cases').children().size()==0)return;var height=$(window).height();$('.tele-body').css({height:height});var offset=height-
$('.tele-header').outerHeight()-
$('.tele-panel-topbar').outerHeight()-
...
setTimeout(function(){telepath.alerts.graphDistributionCanvas.flotGraph({data:pie_data,options:options});$(".tele-alert-graph-distribution-canvas .tele-graph-canvas").bind('plothover',pieHover);$(".tele-alert-graph-distribution-canvas .tele-graph-canvas").bind('plotclick',pieClick);legendClick();set_legend()},100);},show_action_distribution:function(){this.graphDistributionCanvas.empty();var pie_data=this.data.action_distribution_chart;var dataTotal=0;$.each(pie_data,function(i,row){dataTotal+=parseInt(row.data);});var options={series:{pie:{show:true,radius:0.5,innerRadius:0.4,margin:100,label:{show:false,radius:0.3,formatter:function(lbl,obj){var count=parseInt(obj.data[0][1]);var percent=parseInt(count/dataTotal*100)+'%';var pieTemp=$('<div>');var pieLabel=$('<div>').addClass('tele-pie-label').html(lbl);var piePercent=$('<div>').addClass('tele-pie-percent').html(percent).css('color',obj.color);var pieCount=$('<div>').addClass('tele-pie-count').html(count+' alerts');pieTemp.append(pieLabel).append(piePercent).append(pieCount);return pieTemp.html();}}}},grid:{hoverable:true,clickable:true},legend:{show:true,position:"nw",margin:([50,100]),sorted:"ascending",},};function pieHover(event,pos,obj)
{if(!obj)
return;percent=parseFloat(obj.series.percent).toFixed(2);$("#alert-distribution-showPercent").html('<span style="font-weight: bold; color: '+obj.series.color+'">'+obj.series.label+' ('+percent+'%)</span>');$('.tele-graph-canvas .flot-overlay').css({"cursor":"pointer"})}
function pieClick(event,pos,obj){if(!obj)
return;if(obj.series.label){that.filter=[];that.filter.push(obj.series.label)}
...
setTimeout(function(){telepath.alerts.graphDistributionCanvas.flotGraph({data:pie_data,options:options});$(".tele-alert-graph-distribution-canvas .tele-graph-canvas").bind('plothover',pieHover);$(".tele-alert-graph-distribution-canvas .tele-graph-canvas").bind('plotclick',pieClick);legendClick();set_legend()},100);},_resize:function(){if($('.tele-panel-alerts').children().size()==0)return;var height=$(window).height();var width=$(window).width()-10;$('.tele-body').css({height:height});var offset=height-
$('.tele-header').outerHeight()-
$('.tele-panel-topbar').outerHeight()-
$('.tele-panel-subtitle').outerHeight();var magic=500;if(width<1200){if(width<1024){magic=300;}else{magic=400;}}else{magic=600;}
$('.tele-panel-alerts .tele-alert-graphs-block',this.list).width(magic);$('.tele-panel-alerts .tele-block').width(width-magic-15);$('.tele-panel-alerts .tele-block .tele-list').width(width-magic-25).height(offset-50).mCustomScrollbar("update");$('.tele-panel-alerts .tele-alert-graphs-block').height(offset-50).mCustomScrollbar("update");}}
telepath.suspects={sort:'date',dir:false,data:[],total:0,searchString:'',rowFormatter:function(item,mode){if(mode=='dashboard'){var identification=(item.user!=''?{key:'user',value:item.user}:{key:'IP',value:item.ip_orig});result={raw:item,icon:'suspect',time:item.ts,progbar:true,itemID:item.sid,progbarBig:item.progbarBig,checkable:item.checkable,checked:item.checked,count:item.count,progbarValue:item.ip_score,time:item.date,details:[{key:'country',value:item.country},{key:'host',value:grabNames(item.host)},identification]}}
